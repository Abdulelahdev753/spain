        ...
        tab.addEventListener('click', function(e) {
            ...
            if (activeCard) {
                ...
                requestAnimationFrame(() => {
                    ...
                    const cardTop = activeCard.getBoundingClientRect().top + window.pageYOffset;
                    const scrollTop = Math.max(0, cardTop - navbarHeight - daysNavbarHeight - 30);
    ...
    if (daysNavbar) {
        ...
        daysNavbar.addEventListener('touchend', function(e) {
            ...
            if (touchEndX - touchStartX > swipeThreshold && currentDayNum > 1) {
                const prevDayTab = document.querySelector(`.day-tab[data-day="${currentDayNum - 1}"]`);
                if (prevDayTab) prevDayTab.click();
            }
            // Swipe left - next day
            else if (touchStartX - touchEndX > swipeThreshold && currentDayNum < 7) {
                const nextDayTab = document.querySelector(`.day-tab[data-day="${currentDayNum + 1}"]`);
    ...
    dayTabs.forEach((tab, index) => {
        ...
        tab.addEventListener('keydown', function(e) {
            if (e.key === 'ArrowRight' || e.key === 'ArrowLeft') {
                ...
                if ((e.key === 'ArrowRight' && document.dir !== 'rtl') ||
